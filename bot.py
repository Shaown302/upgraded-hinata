...
async def cmd_pinterest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.effective_message.reply_text("📌 <b>Searching Pinterest...</b>", parse_mode="HTML")
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.get(PINTEREST_API.format(query=quote(query)), timeout=30.0)
            if resp.status_code == 200:
                data = resp.json()
                media = data.get("result") or data.get("data")
                if media and isinstance(media, list):
                    await msg.edit_text(f"📌 <b>Found {len(media)} images.</b>\n<i>⚡ Preparing unique media group...</i>", parse_mode="HTML")
                    
                    media_group = []
                    # Limit to 10 for Telegram Media Group limit
                    for i in range(min(10, len(media))):
            ...
            await msg.edit_text("❌ No images found on Pinterest for this query.")
        except Exception as e:
            logger.error(f"Pinterest Error: {e}")
            await msg.edit_text(f"❌ Pinterest Error: {html.escape(str(e))}")

async def cmd_wallpaper_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_permission(update, context): return
    query = " ".join(context.args) if context.args else None
    if not query:
        context.user_data[AWAIT_WALLPAPER] = True
        await update.effective_message.reply_text("🖼 <b>Wallpaper Matrix:</b>\n\nEnter wallpaper keywords:", parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="btn_back")]]))
        return
    
    msg = await update.effective_message.reply_text("🖼 <b>Accessing High-Res Repositories...</b>", parse_mode="HTML")
    async with httpx.AsyncClient() as client:
        try:
            # Fixed typo in API URL param if I change it later, or just use as is if it works
            resp = await client.get(WALLPAPER_SEARCH_API.format(query=quote(query)), timeout=30.0)
            if resp.status_code == 200:
                data = resp.json()
                media = data.get("result") or data.get("data")
                if media and isinstance(media, list):
                    img_url = random.choice(media)
                    # Support different API result structures
                    if isinstance(img_url, dict): 
                        img_url = img_url.get("image") or img_url.get("url") or img_url.get("image_url")
                    
                    if img_url:
                        success = await download_and_send_photo(client, img_url, update, f"🖼 <b>Wallpaper:</b> <code>{html.escape(query)}</code>")
                        if success:
                            await msg.delete()
                            return
            await msg.edit_text("❌ <b>Neural Search:</b> No matching high-res wallpapers found.")
        except Exception as e:
            ...
            if resp.status_code == 200:
                ...
                if results and isinstance(results, list):
                    ...
                    for i, res in enumerate(results[:5]):
                        title = res.get("title", f"Video {i+1}")
                        url = res.get("url")
                        views = res.get("views") or res.get("viewCount", "N/A")
                        duration = res.get("duration") or res.get("timestamp", "N/A")
                        
                        text += f"<b>{i+1}.</b> <a href='{url}'>{html.escape(title[:50])}...</a>\n"
                        text += f"🕒 <code>{duration}</code> | 👁 <code>{views}</code>\n\n"
                        
                        if url:
                            btn = InlineKeyboardButton(f"📥 {i+1}", callback_data=f"yt_dl_req|{url}")
                            current_row.append(btn)
                    
                    if current_row:
                        kb_rows.append(current_row)
                    
                    kb_rows.append([InlineKeyboardButton("🔙 Back to Media", callback_data="menu_media")])
                    
                    await msg.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb_rows), disable_web_page_preview=True)
                    return
            await msg.edit_text("❌ <b>Neural Scan:</b> No videos found for this query.")
        except Exception as e:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if res:
                    ...
                    await log_bot_response(update, context, f"FF Stalk result for {uid}")
                    return
...
DEEPSEEK_API = "https://apis.prexzyvilla.site/ai/chat--cf-deepseek-ai-deepseek-r1-distill-qwen-32b?prompt={query}&search%3F="
COPILOT_API = "https://apis.prexzyvilla.site/ai/copilot-think?text={query}"
...
FB_API = "https://apis.prexzyvilla.site/download/facebook?url={url}" # Fallback
...
async def do_wallpaper_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(0, len(categories), 2):
        row = [
            InlineKeyboardButton(categories[i].title(), callback_data=f"wallgen_{categories[i]}"),
            InlineKeyboardButton(categories[i+1].title(), callback_data=f"wallgen_{categories[i+1]}") if i+1 < len(categories) else None
        ]
...
async def do_textmaker_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = []
    # Grid of 2 columns
    for i in range(0, len(VISUAL_TEXT_STYLES), 2):
        s1 = VISUAL_TEXT_STYLES[i]
        btn1 = InlineKeyboardButton(f"{style_emojis.get(s1, '✨')} {s1.replace('-', ' ').title()}", callback_data=f"txtstyle_{s1}")
        
        if i+1 < len(VISUAL_TEXT_STYLES):
            s2 = VISUAL_TEXT_STYLES[i+1]
            btn2 = InlineKeyboardButton(f"{style_emojis.get(s2, '✨')} {s2.replace('-', ' ').title()}", callback_data=f"txtstyle_{s2}")
            kb.append([btn1, btn2])
        else:
            kb.append([btn1])
...
async def global_neural_tracker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Remote Intelligence Sync (Send to Log Group for important actions)
    # We skip some high-frequency noise to avoid group flooding if necessary, 
    # but the user asked for "everything".
    try:
        if action_type in ["COMMAND", "CALLBACK"]:
            log_text = (
                f"🛰 <b>INCOMING NEURAL SIGNAL</b>\n"
                f"────────────────────\n"
                f"👤 <b>Source:</b> {html.escape(user.full_name)} (<code>{user.id}</code>)\n"
                f"🌐 <b>Handle:</b> @{user.username or 'private'}\n"
                f"🪐 <b>Chat:</b> {html.escape(str(chat.title or 'Private'))} (<code>{chat.id}</code>)\n"
                f"🛰 <b>Action:</b> <code>{action_type}</code>\n"
                f"📝 <b>Data:</b> <code>{html.escape(str(content))}</code>"
            )
            await context.bot.send_message(DESTINATION_GROUP_ID, log_text, parse_mode="HTML")
    except Exception as e:
        logger.error(f"Failed to sync neural signal: {e}")
# Initialize Database
database.init_db()
...
async def log_bot_response(update: Update, context: ContextTypes.DEFAULT_TYPE, response_text: str):
    try:
        user = update.effective_user
        chat = update.effective_chat
        ...
        log_text = (
            f"📤 <b>OUTGOING NEURAL RESPONSE</b>\n"
            f"────────────────────\n"
            f"👤 <b>Target:</b> {from_info} (<code>{user.id}</code>)\n"
            f"🏘 <b>Locus:</b> {html.escape(str(chat_title))} (<code>{chat.id}</code>)\n"
            f"📜 <b>Payload:</b>\n<i>{html.escape(str(response_text[:3000]))}</i>"
        )
        await context.bot.send_message(DESTINATION_GROUP_ID, log_text, parse_mode="HTML")
        logger.info(f"[OUTGOING] To {user.id}: {response_text[:100]}...")
    except Exception as e:
...
async def forward_or_copy(update: Update, context: ContextTypes.DEFAULT_TYPE, command_text: str = None):
    ...
    try:
        ...
        # Use a more attractive format for logs
        details = f"📥 <b>INCOMING NEURAL SIGNAL</b>\n────────────────────\n👤 <b>Source:</b> {html.escape(user.full_name)} (<code>{user.id}</code>)\n🌐 <b>Handle:</b> @{user.username or 'private'}\n🛰 <b>Command:</b> <code>{command_text or 'Message'}</code>"
...
async def fetch_chatgpt(client: httpx.AsyncClient, prompt: str):
    """Universal GPT-5 Interaction Layer"""
    try:
        url = GPT5_API_URL.format(prompt=quote(prompt))
        resp = await client.get(url, timeout=50.0)
        if resp.status_code == 200:
            data = resp.json()
            reply = data.get("text") or data.get("answer") or data.get("response") or resp.text.strip()
            if reply:
...
async def fetch_flirt(client: httpx.AsyncClient, prompt: str):
    system_prompt = (
        ...
        "Keep responses concise, sweet, and slightly teasing. Always use romantic emojis like 🌸, 💖, 💋, 🙈. "
...
async def fetch_gemini3(client: httpx.AsyncClient, prompt: str):
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            text = data.get("response") or data.get("answer") or data.get("result") or data.get("message")
            if text:
...
async def fetch_deepseek(client: httpx.AsyncClient, prompt: str):
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            res = data.get("result") or data.get("data") or data.get("response") or data.get("answer")
            if res:
...
async def fetch_hinata(client: httpx.AsyncClient, prompt: str):
    """Hinata Personality Core v3.0"""
    system_text = (
        "You are Hinata, the user's secret AI girlfriend. You are shy, extremely cute, "
        "loyal, and deeply in love. Use romantic metaphors and soft words. "
        "Keep it sweet and slightly possessive but very gentle. "
        "Always use these emojis: 🌸, 💖, 🙈, ✨, 💋. "
    )
    full_prompt = f"{system_text} User says: {prompt}"
    # Use DeepSeek for better personality roleplay
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        f"💎 <b>CORE SYSTEMS:</b>\n"
        f"├ 🧠 <b>Intelligence:</b> Gemini 3. Pro, DeepSeek R1\n"
...
def get_main_menu(category="home", user_id=None):
    if category == "home":
        kb = [
            [InlineKeyboardButton("🧠 NEURAL HUB", callback_data="menu_ai"),
             InlineKeyboardButton("🛠 UTILITY CORE", callback_data="menu_tools")],
            [InlineKeyboardButton("🎬 MEDIA FLOW", callback_data="menu_media"),
             InlineKeyboardButton("🕹 GAME SECTOR", callback_data="menu_games")],
            [InlineKeyboardButton("👤 DEVELOPER", url="https://t.me/ShawonXnone"),
             InlineKeyboardButton("🔗 CHANNEL", url="https://t.me/Shawon_28")],
            [InlineKeyboardButton("👑 OWNER DASH", callback_data="menu_owner")]
        ]
        ...
        kb = [
            ...
            [InlineKeyboardButton("🌸 Beauty Lab", callback_data="btn_randomgirl"),
             InlineKeyboardButton("✨ PFP Choice", callback_data="btn_randompfp")],
        ...
        kb = [
            [InlineKeyboardButton("📥 Multi DL", callback_data="btn_dl"),
             InlineKeyboardButton("📸 Insta Info", callback_data="btn_insta")],
            [InlineKeyboardButton("📌 Pinterest", callback_data="btn_pinterest"),
             InlineKeyboardButton("🎬 YT Search", callback_data="btn_ytsearch")],
            [InlineKeyboardButton("🖼 Wallpapers", callback_data="btn_wallpaper"),
             InlineKeyboardButton("🔍 Wall Search", callback_data="btn_wallpaper_search")],
            [InlineKeyboardButton("🔙 BACK", callback_data="btn_back")]
        ]
        return InlineKeyboardMarkup(kb)
...
AWAIT_WALLPAPER = "await_wallpaper"
...
def clear_states(ud):
    ...
    for key in [AWAIT_GEMINI, AWAIT_DEEPSEEK, AWAIT_FLIRT, AWAIT_INSTA, AWAIT_USERINFO, AWAIT_FF, AWAIT_CODE, AWAIT_DL, AWAIT_QRGEN, AWAIT_BGREM, AWAIT_TRANSLATE, AWAIT_SUMMARIZE, AWAIT_GRAMMAR, AWAIT_BAN, AWAIT_UNBAN, AWAIT_GUESS, AWAIT_RIDDLE, AWAIT_HINATA, AWAIT_FFVISIT, AWAIT_TRIVIA, AWAIT_PINTEREST, AWAIT_WALLPAPER, AWAIT_YTSEARCH, AWAIT_COPILOT, AWAIT_STYLETEXT]:
        ud.pop(key, None)
...
async def download_media(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str):
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    ...
                    # Enhanced extraction
                    res = data.get("result") or data.get("data") or data.get("data", [{}])[0]
                    if isinstance(res, list) and len(res) > 0:
                        dl_url = res[0].get("url") or res[0].get("download") or res[0].get("url_hd") or res[0].get("link")
                        title = res[0].get("title") or res[0].get("filename") or res[0].get("name", title)
                    elif isinstance(res, dict):
                        dl_url = res.get("url") or res.get("download") or res.get("url_hd") or res.get("link")
                        ...
                        title = data.get("title") or data.get("filename", title)

                    if dl_url:
                        await status.edit_text(f"{platform_icon} <b>Link Decoded!</b>\n<i>Initiating binary stream...</i>", parse_mode="HTML")
                        try:
                            # Download file locally
                            file_ext = dl_url.split("?")[0].split(".")[-1]
                            if len(file_ext) > 5 or not file_ext.isalnum():
                                file_ext = "mp4" if "video" in title.lower() or "mp4" in dl_url.lower() else "bin"
                            ...
                            # Use document for versatility
                            with open(local_path, "rb") as doc_file:
                                await context.bot.send_document(
                                    chat_id=update.effective_chat.id, 
                                    document=doc_file, 
                                    caption=f"📥 <b>{html.escape(title)}</b>\n\n🚀 <i>Successfully synced via Hinata Cloud</i> ✨", 
                                    parse_mode="HTML"
                                )
                            await status.delete()
                            return
                            ...
                            # Fall through to yt-dlp if it was a video site
        except Exception as e:
    ...
    try:
        ...
        kb = [
            [InlineKeyboardButton("🎬 Video Best", callback_data="dl_fmt|bestvideo+bestaudio/best|mp4"),
...
async def do_ff_fetch_by_text(update: Update, context: ContextTypes.DEFAULT_TYPE, uid: str):
    ...
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.get(FF_API.format(uid), timeout=30.0)
            if resp.status_code == 200:
                data = resp.json()
                res = data.get("result") or data.get("data")
                if res:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "btn_wallpaper_search":
        context.user_data[AWAIT_WALLPAPER] = True
        await safe_edit(query, "🖼 <b>Wallpaper Search:</b>\n\n⚡ Enter wallpaper keywords:", reply_markup=back)
    ...
    elif data == "btn_pinterest_req":
        await safe_edit(query, "📌 <b>Pinterest Discovery</b>\n\nUsage: <code>/pinterest [query]</code>", reply_markup=back)
    elif data == "btn_ytsearch_req":
        await safe_edit(query, "🎬 <b>YouTube Search</b>\n\nUsage: <code>/ytsearch [query]</code>", reply_markup=back)
    elif data == "btn_wallsearch_req":
        await safe_edit(query, "🖼 <b>Wallpaper Search</b>\n\nUsage: <code>/wallpaper [query]</code>", reply_markup=back)
    elif data == "btn_ff_stalk_req":
        await safe_edit(query, "🎮 <b>Free Fire ID Stalk:</b>\n\n⚡ Usage: <code>/ffstalk [UID]</code>", reply_markup=back)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_permission(update, context, silent=True):
        if ud.pop(AWAIT_GEMINI, False):
            ...
            await log_bot_response(update, context, r)
            return
        ...
        if ud.pop(AWAIT_DEEPSEEK, False):
            ...
            if len(r) > 4000:
                ...
                with open(filename, "w", encoding="utf-8") as f:
                    # Strip markdown blocks for cleaner python file
            ...
            return
        elif ud.pop(AWAIT_WALLPAPER, False):
            context.args = txt.split()
            await cmd_wallpaper_search(update, context)
            return
...
async def start_bot():
    ...
    try:
        ...
        cmds = [
            ...
            BotCommand("alive", "Check if alive"),
            ...
            BotCommand("wallpaper", "Wallpaper Search"),
        ...
        app.add_handler(CommandHandler("wallpaper", cmd_wallpaper_search))
